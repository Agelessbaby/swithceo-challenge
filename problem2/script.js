...
const calculateSwapAmount = () => {
  ...
  if (isNaN(inputAmount) || inputAmount <= 0) {
    document.getElementById('error-message').textContent = "Please enter a valid amount greater than 0.";
    document.getElementById('output-amount').value = "";  // 清空输出框
    return; // 退出函数，不进行计算
  }
  ...
  if (!fromCurrencyData || !toCurrencyData) {
    document.getElementById('error-message').textContent = "Invalid currencies selected.";
    document.getElementById('output-amount').value = "";  // 清空输出框
    return; // 退出函数，不进行计算
  }
  ...
  document.getElementById('error-message').textContent = ""; // 清空错误消息
};
